...
$(document ).ready(function() {
  ...
  function updateBalance() {
    $("#balance").empty();
    $("#balance").append('<h2 class="text-center">' + "Balance: " + formatAmount(account.balance) + ' WON</h2>');
  }
  ...
  $("form#add-account").submit(function(event) {
    ...
    $("#title").empty();
    $("#title").append('<h1 class="text-center" id="title">SIN Bank System' + '!</h1>');
    $("#bank").hide();
    $("#account").show();
    updateBalance();
    clearForms();
  ...
  $("form#deposit").submit(function(event) {
    ...
    updateBalance();
    var date = new Date();
    $("table#history").append('<tr class="success"> <td>' + date.toLocaleString() + '</td> <td> +' + deposit.toString() + ' WON</td> <td>' + (account.balance).toString() + ' WON</td> </tr>');
    clearForms();
    $("#history-panel").show();
  });
  ...
  $("form#withdraw").submit(function(event) {
    ...
    updateBalance();
    var date = new Date();
    $("table#history").append('<tr class="danger"> <td>' + date.toLocaleString() + '</td> <td> -' + withdrawal.toString() + ' WON</td> <td>' + (account.balance).toString() + ' WON</td> </tr>');
    clearForms();
    $("#history-panel").show();
  });
